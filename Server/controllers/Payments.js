...
exports.capturePayment = async(req,res)=>{
    ...
    try{
        ...
        console.log(error);
        return res.status(500).json({success:false, message:"Could not create initaiate order"})
    }
}
...
const enrollStudent = async(courses,userId,res)=>{
    try{
        ...
        for(const courseId of courses){
            ...
            const emailResponse = await mailSender(enrolledStudent.email,`Successfully Enrolled into ${enrolledStudent.courseName}`,
...
exports.sendPaymentSuccessEmail = async(req,res) =>{
    ...
    try{
        ...
        await mailSender(enrollStudent.email,`Payment Received`,this.paymentSuccessEmail(`${enrollStudent.firstName}`,amount/100,orderId,paymentId))
    }catch(error){
        console.log("error in sending mail"); 
            return res.status(500).json({success:false,message:"cound not sent email"})
    }
