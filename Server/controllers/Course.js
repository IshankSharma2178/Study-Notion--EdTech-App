...
exports.createCourse = async (req, res) => {
  try {
    const {
      ...
      tag,
      instructions,
    } = req.body; //Category m id ayegi becoz course model m Category ref le rkha hai
    ...
    const addCourseToCategory = await Category.findByIdAndUpdate(
      { _id: category },
...
exports.editCourse = async (req, res) => {
  try {
    ...
    if (thumbnailImage) {
      try {
        var thumbnailResponse = await uploadFileToCloudinary(
          thumbnailImage,
          "codehelp"
        );
      } catch (Err) {
...
exports.deleteCourse = async (req, res) => {
  try {
    ...
    category.course = category.course.filter(
      (cour) => cour._id.toString() !== courseId.toString()
    );
    ...
    const deleteSubSectionsPromises = course.courseContent.map((section) => {
      return Promise.all(
        section.subSection.map(async (subSection) => {
    ...
    const courseRatings = await RatingAndReview.deleteMany({
      course: courseId,
    });

    const courseComments = await Comment.deleteMany({ course: courseId });

    const courseProgress = await CourseProgress.deleteMany({
      courseID: courseId,
    });
