...
function SidebarLink({ link, iconName }) {
    ...
    return (
        <NavLink onClick={setDataZero}
         to={link.path} className={`relative px-8 md:mx-0 mx-2 py-2 text-sm font-medium ${matchRoute(link.path) ? "bg-yellow-800" : " bg-opacity-0"}`}>
            ...
            <div className={`${matchRoute(link.path) ? "text-yellow-50" : ""} flex items-center text-richblack-300 gap-x-2`}>
                {Icon && <Icon className=" md:text-lg text-xl" />}
                <span className='md:text-lg text-xl'>{link.name}</span>
            </div>
        </NavLink>
    );
}
