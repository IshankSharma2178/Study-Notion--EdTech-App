      ...
      {loadng ? (
        ...
        cart.map((course, index) => {
          return (
            <div
              key={course._id}
              className={`flex  w-[100%] flex-wrap items-start justify-between gap-6 ${
                index !== cart.length - 1 &&
                "border-b border-b-richblack-400 pb-6"
              } ${index !== 0 && "mt-6"} `}
            >
              <div
                className="flex flex-1 flex-col gap-4 md:flex-row cursor-pointer"
              ...
              <div className="flex flex-col items-end space-y-2">
                <button
                  className="flex items-center gap-x-1 rounded-md  my-3 mx-[2px] text-pink-200"
