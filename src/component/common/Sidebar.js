...
function Sidebar({ setShowSidebar, showSidebar }) {
  ...
  return (
    <div
      className={`fixed top-0 left-0 h-screen Sidebar-bg z-50 transition-transform duration-300  ease-in ${
        showSidebar ? "translate-x-0" : "-translate-x-full"
      } w-[75vw] bg-white text-black`}
      ref={sidebarRef}
    >
      <div className="flex flex-col h-full p-4">
        ...
        {token !== null ? (
          <div className="mt-6 flex flex-row justify-between gap-3  relative min-h-[100px]">
            <img
              ...
              loading="lazy"
              alt="User"
              className="outline outline-richblack-600  p-1 w-14 h-14 rounded-full"
            />
        ...
        <div className="flex flex-col gap-4">
          <NavLink to={NavbarLinks[0].path} onClick={() => setShow(false)}>
            <div
              className={`flex rounded-md flex-row gap-4 items-center py-2 pl-3 ${
                !show && matchRoute(NavbarLinks[0].path)
                  ? "bg-yellow-25 text-richblue-900 text-[17px]"
                  : "text-[17px] text-richblack-25"
              }`}
            >
              <RiHome4Line className="text-2xl font-bold" />
              <p className="text-xl font-semibold">{NavbarLinks[0].title}</p>
            </div>
          </NavLink>
          ...
          {user?.accountType === "Student" && (
            ...
            >
              <div
                className={`flex rounded-md flex-row gap-4 items-center py-2 pl-3 ${
                  !show && matchRoute("dashboard/enrolled-courses")
                    ? "bg-yellow-25 text-richblue-900 text-[17px]"
                    : "text-[17px] text-richblack-25"
                }`}
              >
          ...
          <div onClick={() => setShow(true)} className="flex flex-col">
            ...
            >
              ...
              <p className="text-xl font-semibold">{NavbarLinks[1].title}</p>
            </div>
            <div
                ...
                (subLinks?.length ? (
                  subLinks.map((sublink, index) => (
                    <div>
                      ...
                      >
                        <p
                          className={`text-richblack-100 text-xl font-semibold pl-4 ${
                            matchRoute(
                              "/catalog/" +
                                sublink.name.split(" ").join("-").toLowerCase()
                            )
                              ? "text-yellow-5 bg-none"
                              : ""
                          } `}
                        >
                          {sublink.name}
          ...
          <NavLink to={NavbarLinks[2].path} onClick={() => setShow(false)}>
            <div
              className={`rounded-md flex flex-row gap-4 items-center py-2 pl-3 ${
                !show && matchRoute(NavbarLinks[2].path)
                  ? "bg-yellow-25 text-[17px]"
                  : "text-[17px] text-richblack-25"
              }`}
            >
              <IoInformationCircleOutline className="font-bold text-2xl" />
              <p className="font-semibold text-xl">{NavbarLinks[2].title}</p>
            </div>
          </NavLink>
          <NavLink to={NavbarLinks[3].path} onClick={() => setShow(false)}>
            <div
              className={`rounded-md flex flex-row gap-4 items-center py-2 pl-3 ${
                !show && matchRoute(NavbarLinks[3].path)
                  ? "bg-yellow-25 text-[17px]"
                  : "text-[17px] text-richblack-25"
              }`}
            >
              <MdOutlinePermPhoneMsg className="font-bold text-2xl" />
              <p className="font-semibold text-xl">{NavbarLinks[3].title}</p>
            </div>
          </NavLink>
          {token === null && (
          ...
          {token && (
            <button onClick={() => logout()}>
              <div
                className={`flex rounded-md flex-row gap-4 items-center py-2 pl-3 ${
                  !show && matchRoute("/enrolled-courses")
                    ? "bg-yellow-25 text-richblue-900 text-[17px]"
                    : "text-[17px] text-richblack-25"
                }`}
              >
